# CI-LINT-TYPECHECK-TEST — CI: Lint·타입체크·테스트 게이트
...
**CI: Lint·타입체크·테스트 게이트**. CI 파이프라인 또는 배포 플랫폼(Vercel/Supabase) 설정을 확인하는 Task다.
...
- main 병합 전 `next lint`, `tsc --noEmit`, 단위/통합 테스트가 모두 통과해야 병합 가능.
