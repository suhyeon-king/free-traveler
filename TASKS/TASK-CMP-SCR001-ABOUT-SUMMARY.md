# CMP-SCR001-ABOUT-SUMMARY — free_traveler 요약 배너
...
**free_traveler 요약 배너**. SCR-001 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
`design-reference/D-001/DESIGN.md` — Section별 제목·설명·본문·CTA 계층
...
- `src/components/scr-001/AboutSummary.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 대표명·`50+ Trips`·`30+ Countries`를 `/about`과 동일 값으로 표시, "대표 소개 더 보기" CTA.

## Visual AC

- 좌우 분할(사진+텍스트), Desktop/Mobile 모두 자연스러운 한국어 완성 문장.
