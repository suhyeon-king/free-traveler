# CMP-SCR001-MATE-PREVIEW — 최근 동행글 미리보기 + Empty State
...
**최근 동행글 미리보기 + Empty State**. SCR-001 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
`design-reference/D-001/DESIGN.md` — Mate Post Card / Loading·Empty·Error 상태
...
- `src/components/scr-001/MatePreview.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 모집중 동행글 최신 3개(제목/국가·기간/모집 상태)와 "동행 더 보기" 링크. 0건 시 완성형 Empty State(이유+이용 방법+작성 CTA). 종료일 경과 글은 조회 시 자동 마감 표시로 계산.

## Visual AC

- 3개 카드 또는 Empty State, 큰 빈 여백 없음.
