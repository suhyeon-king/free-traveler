# CMP-SCR002-COUNTRY-CHIPS — 방문 국가 Chip(권역별)
...
**방문 국가 Chip(권역별)**. SCR-002 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
`design-reference/D-001/DESIGN.md` — Search·Filter(Chip) / Spacing
...
- `src/components/scr-002/CountryChips.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 아시아/유럽/북미/오세아니아 권역별로 국가 Chip을 그룹화하고 클릭 시 SCR-001 관련 여행지/안전정보로 이동.

## Visual AC

- 총 30개국 이상, 연결 오류 0건. Mobile은 권역별 가로 스크롤.
