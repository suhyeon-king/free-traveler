# CMP-SCR003-HOTEL-FORM — 숙소 탭(입력·검증·요약·외부이동·Tip)
...
**숙소 탭(입력·검증·요약·외부이동·Tip)**. SCR-003 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
- `src/components/scr-003/HotelTab.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 국가·지역·체크인·체크아웃 필수 입력, 국가 변경 시 지역 재계산, 체크아웃≤체크인 차단, 요약+비전달 고지, "숙소 보러 가기" 새 탭 이동, Tip 3개.
...
- 입력값 서버 미저장·미전달. 외부 이동 새 탭+`noopener,noreferrer`.

## Test Cases

- TC-FUNC-019 (REQ-FUNC-019)
- TC-FUNC-020 (REQ-FUNC-020)
- TC-FUNC-021 (REQ-FUNC-021)
- TC-FUNC-022 (REQ-FUNC-022)
- TC-FUNC-023 (REQ-FUNC-023)
- TC-FUNC-024 (REQ-FUNC-024)
- TC-FUNC-025 (REQ-FUNC-025)
