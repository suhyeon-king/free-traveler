# CMP-SCR004-APPLY — 참가 요청 제출 + 중복 차단
...
**참가 요청 제출 + 중복 차단**. SCR-004 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
`design-reference/D-001/DESIGN.md` — Form·Tabs / Alert·Toast
...
- `src/components/scr-004/ApplyForm.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 500자 이내 참가 메시지 비공개 제출, PENDING 저장, 동일 글 중복 PENDING/ACCEPTED 차단(DB unique + UI 오류).

## Visual AC

- 제출 완료 Toast, p95 3초 이내 응답 목표.

## Security/Privacy AC

- 비로그인/성인 미확인 시도는 안내 카드로 전환(서버 401 이중 방어).
