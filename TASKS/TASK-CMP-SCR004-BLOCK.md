...
**차단 트리거**. SCR-004 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
- `src/components/scr-004/BlockButton.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 차단 실행 시 즉시 상호 글·프로필·요청 노출 제한(관리는 SCR-005 내 활동 탭).
