# CMP-SCR004-DETAIL — 상세 패널(Desktop)/Drawer(Mobile)
...
**상세 패널(Desktop)/Drawer(Mobile)**. SCR-004 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
`design-reference/D-001/DESIGN.md` — Mate Post Card / Drawer·Modal
...
- `src/components/scr-004/MateDetailPanel.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 작성자 표시 정보·조건·설명·모집 상태 노출, 연락처·이메일 절대 비노출, URL 공유 지원.

## Visual AC

- Desktop 목록(30%)+상세(70%) 고정 분할, Mobile Bottom Sheet Drawer.

## Security/Privacy AC

- API 응답 JSON/HTML에 이메일·전화번호 미포함.
