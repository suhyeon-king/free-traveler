...
**검색 Filter + 결과 요약**. SCR-004 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
- `src/components/scr-004/MateFilter.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 국가·지역·기간·연령대·성별·여행 스타일·모집 상태 필터, 결과 "총 N개" 요약 텍스트, 차단 사용자 글 제외.

## Visual AC

- p95 1초 이내 결과(로컬 캐시/필터).
