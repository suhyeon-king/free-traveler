# CMP-SCR005-AUTH — 로그인·가입·재설정(Guest)
...
**로그인·가입·재설정(Guest)**. SCR-005 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
- `src/components/scr-005/AuthTab.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 이메일 가입·인증·로그인·로그아웃·비밀번호 재설정, 로그인 후 가능한 기능 안내, 보안 안내(생년월일 미저장).
