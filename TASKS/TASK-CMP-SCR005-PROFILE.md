# CMP-SCR005-PROFILE — 프로필·성인 확인 요약(Member)
...
**프로필·성인 확인 요약(Member)**. SCR-005 화면 내 특정 영역을 구현하는 Component Task다. 이 Task는 화면 전체가 아니라 지정된 Expected Files 범위의 UI 조각만 책임진다.
...
- `src/components/scr-005/ProfileTab.tsx`

**이 Task는 위에 나열된 파일만 신규 생성하거나 수정한다. 목록 밖의 파일은 수정하지 않는다.**

## Functional AC

- 닉네임·연령대·여행 스타일(필수)·성별(선택)·성인 확인 완료 배지, 프로필 저장.
...
- 정확한 생년월일 미저장, `is_adult`+확인 시각만 저장.
