...
**여행지 정적 데이터(국내 10+, 해외 15개국 30도시+)**. 정적 데이터(`src/data`)를 정의·작성하는 Task다. 별도 관리자 CRUD 화면 없이 코드로 직접 관리한다.
...
- `src/data`에 국내 10개 이상·해외 15개국 30개 도시 이상, 각 항목에 소개 300자↑·명소 5개↑·추천 시기·1일/3일 일정·예산·교통·음식 3개↑·에티켓·출처·수정일 필드를 스키마로 강제.

## Visual AC

- 이미지 필드는 URL+alt만(라이선스 메타 간소화).
...
- TC-FUNC-008 (REQ-FUNC-008)
- TC-FUNC-009 (REQ-FUNC-009)
- TC-NF-026 (REQ-NF-026)
