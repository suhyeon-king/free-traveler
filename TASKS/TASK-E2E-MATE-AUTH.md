# E2E-MATE-AUTH — 동행 인증 흐름 Smoke(로그인/참가/계정, 2개 흐름)
...
**동행 인증 흐름 Smoke(로그인/참가/계정, 2개 흐름)**. Playwright(Chromium)로 핵심 사용자 흐름을 검증하는 End-to-End 테스트 Task다.
...
- **Route:** `/mates`, `/account`
- **Page Entry:** `src/app/mates/page.tsx`, `src/app/account/page.tsx`
...
- 흐름6: 로그인→동행글 상세→참가 요청 제출→작성자 계정에서 승인. 흐름7: 계정 페이지 역할별 탭 노출(Guest/Member) 확인. Chromium만 사용.
