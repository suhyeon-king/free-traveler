...
> 2. **상세 Drawer 미연동 → 이후 해결됨(W12 RELEASE-CHECK-EXTERNAL-LINKS 준비 중 발견·수정)**: 처음에는 여행지 카드/안전정보 카드를 클릭해도 Drawer가 열리지 않는 상태로 이 Task를 완료했다(당시 사람 확인 후 이 Task 범위에서 제외). 이후 `RELEASE-CHECK-EXTERNAL-LINKS`(외교부 안전정보 링크 확인) 체크리스트를 준비하다가, 이 미연동 때문에 REQ-FUNC-049(외교부 링크)가 사람이 실제로 확인할 수조차 없는 상태임을 발견해 사람에게 다시 확인받고 해결했다. 새 Client Wrapper를 만들지 않고, `PAGE-SCR001`(server)이 `?destination=<id>`/`?safety=<countrySlug>` 쿼리 파라미터로 `DestinationDrawer`/`SafetyDrawer`를 열며, 두 Drawer와 `DestinationGrid`/`SafetyGrid`의 콜백 prop을 전부 optional로 바꿔 생략 시 각 Component가 `useRouter()`/`useSearchParams()`로 스스로 그 파라미터를 읽고 써서 열고 닫도록 했다(`CMP-SCR001-DESTINATIONS`/`CMP-SCR001-SAFETY-PANEL`의 Expected Files, 이 Task 밖이지만 이미 완성된 Component를 조립하는 범위 안에서 최소 수정, 사람 확인 완료). `npm run build` PASS, Playwright로 카드 클릭 → Drawer 열림(`?destination=`/`?safety=` URL 반영) → 외교부 "원문 보기" 링크 노출까지 실제 확인했다.
