...
**동행 조회 페이지 조립**. 승인된 디자인 Screen SCR-004을 실제 Next.js Route Page로 조립하는 Page Owner Task다.
...
`design-reference/D-001/DESIGN.md` — Header·Footer / Search·Filter / Mate Post Card / Drawer·Modal / 완성형 Empty State 규칙
...
- Section 순서를 Intro+작성 CTA → Filter·결과 요약 → 동행 목록 → 상세 → 신청 방법 3단계 → 안전·신고·차단 안내+CTA 순으로 재현한다.
- 데이터 출처: 목록·필터=DB-ACCESS(mate_posts), 상세=DB-ACCESS+CMP-SCR004-DETAIL, 참가/신고/차단=DB-ACCESS mutation.
- Desktop은 목록+상세 2열 분할, Mobile은 목록→상세 Drawer.
- `design-reference/UI_CONTRACT.md` SCR-004 상태 정의를 그대로 구현한다: Loading(Filter 결과 로드 중 목록 스켈레톤) · Success · Empty(필터 결과 0건 시 필터 초기화+작성 CTA+이용 방법이 있는 완성형 Empty State) · Error(참가 요청 중복 제출, 신고·차단 처리 실패 시 인라인 오류) · Unauthorized(비로그인/성인 미확인 시 참가 요청·신고·차단 트리거 대신 안내 카드로 전환).

## Visual AC

- 목록은 데이터가 있으면 최대 8개 우선 노출. 결과 0건 시 필터 초기화+작성 CTA+이용 방법이 있는 완성형 Empty State.
- Lorem ipsum·"준비 중"·내용 없는 Card 금지.

## Security/Privacy AC

- 참가 요청·신고·차단은 로그인+성인 확인된 사용자만 가능(비로그인 시도는 안내 카드로 전환, 서버 401/RLS로 이중 차단). 모집글 응답에 연락처 필드 미포함.
