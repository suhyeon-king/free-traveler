...
/**
 * 국가→지역(도시) 목록. 이 Tab만의 로컬 데이터다(다른 Task 소유 파일을 늘리지
 * 않기 위해 여기서만 관리하며, 실제 목적지 데이터와는 별개다).
 */
...
const OFFICIAL_DISCLAIMER =
  "이 요약은 공식 판단을 대체하지 않습니다. 실제 이동 전 항공사 공식 페이지에서 최신 정보를 다시 확인하세요.";
const TIPS = [
  "출발/도착 요일에 따라 요금이 달라질 수 있으니 며칠 전후로도 함께 확인해 보세요.",
  "왕복보다 편도 조합이 유리한 경우가 있으니 비교해 보세요.",
...
interface FlightTabProps {
  /** `flight_outbound_url`(app_settings) 또는 `FLIGHT_OUTBOUND_URL` 환경변수 값.
   * 서버에서 미리 해석해 문자열로 전달한다(Server→Client 직렬화 제약으로 이 파일
   * 안에서 직접 조회하지 않는다). */
 ...
 * DB·로그·쿼리로 전송하지 않는다.
 */
export function FlightTab({ outboundUrl }: FlightTabProps) {
  ...
  function handleCountryChange(country: string) {
    // 국가 변경 시 지역 옵션을 재계산하고 기존 선택값을 초기화한다(REQ-FUNC-012).
  ...
  if (isSummary) {
    return (
      <div className="flex flex-col gap-4">
        ...
        <p className="text-[13px] text-[#6B6863]">{OFFICIAL_DISCLAIMER}</p>
      </div>
    );
  }
